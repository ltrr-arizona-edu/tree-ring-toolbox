...
for i=1:n; % Loop over trees
    ...
    % Make a version of sample size vector with NaN where zero number of cores
    tntemp=tn;
    Lzero=tn==0; % zero sample size
    if any(Lzero);
        tntemp(Lzero)=NaN;
    end;
...
% commented out: not needed; can use nansum
% 	L6 = ~L5; % logical pointer to NaNs
% 	numz = sum(sum(L6));
% 	zee = zeros(numz,1);
% 	Z(L6)=zee;
