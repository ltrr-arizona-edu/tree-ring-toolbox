...
case 'Cancel';
   klook='No';
case 'Yes';
    uiwait(msgbox('Press Mouse Button to Move from Plot to Next Plot','','modal'));
   ...
   if S(scid,1)~=0,
      ...
      for k=1:2:length(lz)-1,
         ...
         if length(xvt)>=20,
            ...
            % REVISE R12
            th=ar(xvtt,nn); % now and iddata object
            temex=resid(th,iddata(xvtt));
            % temex=resid1(xvtt,th); -- old
            temex=get(temex,'OutputData');
            
            %P(pidx,5)=var(temex)/var(xvtt);
            NoiseVariance=get(th,'NoiseVariance');
            %varrat=th(1,1)/var(xvtt); % ratio of variance of residuals to original variance
            varrat=NoiseVariance/var(xvtt);
            ArParams=get(th,'a');
            ArParams=ArParams(2:length(ArParams));
                        
            P(pidx,5)=varrat;
            %P(pidx,6:5+nn)=th(3,1:nn);  -- OLD
            P(pidx,6:5+nn)=ArParams;
