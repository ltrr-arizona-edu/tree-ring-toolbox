function recplot1(xdatin1,xdatin2,axin,textin1,textin2,color1)
...
%       Note that if a moving average of length m, length reduced by (m-1) from annual reconstructio
%   {2} yrxr1 (nyr1 x 1)i year vector for xr1
%   {3} yrs (3 x 1)i
%         start year of long-term reconstruction, before smoothing
...
%       Note: nyrB covers just the pre-calib period data, minus (m-1)
...
for n = 1:naxes;
   ...
   if any(L);
      x=xa(L);
      yr=yrxa(L);
      plot(yr,x,'color',colthis,'LineWidth',1);
   end;
   ...
   if n==naxes;
      ...
      arrow([xyarrgo(1,2) xyarrgo(2,2)],[xyarrsp(1,2) xyarrsp(2,2)],'TipAngle',10,...
         'Length',10);
      text(xytext(1,4),xytext(2,4),linlab{4},'HorizontalAlignment','right');
