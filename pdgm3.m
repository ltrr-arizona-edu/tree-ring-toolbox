...
% on the smoothed periodogram.  Modified from pdgm1.m to give
% two plots on a frame.  The time series on top, the spectrum on
% bottom.  Assumes you've already run pdgm1.m to get plots
%
...
plot(fyr:1:lyr,X)
 ...
 xlabel('Year')
 ylabel('Index') 
 text(.65,.85,['Region ',int2str(nnn)],'sc');
grid
pause 
keyboard
clc
...
x1over=0.3;      % for plotting text identifying region
y1over = 5*max(ysmhi)/8;
