...
%    R-squared, adjusted-R-squared, and F-level for eqn

F1=stats(3);
dfn1 = length(I2);  dfd1=mz-length(I2);  % numer and denom deg freed.
...
[RR,V,L,S,F,B]=eigen1(Z,1);  % pca on correl mtx of Z
...
dfn2=sum(Jr);  dfd2=mz-sum(Jr);  % deg freedom, numer and denomin
F2=vexp * (mz-sum(Jr)) / ((1-vexp)*(sum(Jr))) ; % F-ration for sample
...
elseif k2==2;  % Summary of mlr, tree vs monthly climate
	...
	text(0.5,ax(3),'DASHED -- 2-SE CONF BANDS');
	pause
	if k4=='Y', keyboard, end;
...
elseif k2==3;  %  PC REGRESSION, TREE VS CLIMATE
	...
	yp= (ax(4)-ax(3))/30;  % compute y plotting pt
	...
	text(0.5,ax(4)-2*yp,['F = ',num2str(F2),'   df= ',...
	  ...
	  num2str(ff2)]);
	text(0.5,ax(4)-3*yp,['SAMPLE SIZE = ',int2str(mz)]);
	pause
...
elseif k2==4;  % Plot individual climate pcs
	...
	if k5==1
		...
		title(['CLIMATE PCs ',int2str(kwhich),...
...
elseif k2==6;  % Quit the plot loop
	break
end
end;  % of while on k2
