...
% Last revised 9-3-99
%
% Writes tree-ring index to an ITRDB-formatted .crn file. You can choose 
% whether .crn file is to contain standard or residual index, and whether 
% sample size should be number of cores or number of trees in any year. You can 
% optionally build an ITRDB-form 3-line header too.
%
%*** INPUT
%
...
%  1st 6 columns of the matrix contain site ID string (e.g., PADSTD)
%  Next 4 columns contain "decade year"
%  Remaining columns are the data and sample size in individual years
%
% Output filename.  If the output file is the standard total ringwidth index, the
% file name is the xxxxxx.crn, where xxxxxx is the 6 char id prompted for. If 
% data is other type, a letter is appended to the first part of the name:
%  xxxxxxr  residual index of total ring width
%  xxxxxxer (not yet implemented) residual index of earlywood width
%
...
% and optionally modifying  the site name, species, and data code.  This way is 
% convenient for making separate files of the "standard" and "residual" chronologies. 
% Then really all that will change in the header info is that "R" will replace the 
% blank as col 63 on line 3, and the start year might be different.  The start and 
% end years are determined from the data itself, so you don't need to manually 
% set them.
...
%-- Load the .mat input file, with the appropriate data type
if strcmp(dtype,'std');
   eval(['load ' pf1 ' ZI yrZI;']);
   Z=ZI;
   yrZ=yrZI;
elseif strcmp(dtype,'r');
...
%--- OPTIONALLY ADD ON A 3-LINE HEADER TO THE FILE
...
elseif khead==2; %ITRDB 3-line header, specify all info
   hdA1=[id ' 1 ']; hdB1=[id ' 2 ']; hdC1=[id ' 3 ']; % left-hand side info
   ...
   splist={'Western Juniper',...
      ...
      'Sugar Pine',...
      'Limber Pine'};
   scodes ={'JUOC','PSME','PIPO','PIMO','PILA','PIFL'};
   ...
   dlgTitle='Collector/Developer/ (max of 52 chars)';
   lineNo=1;
   answer=inputdlg(prompt,dlgTitle,lineNo,def);
   hdC2=answer{1};
   ntemp=length(hdC2);
...
elseif khead==3; % modify header off an existing .crn file
   ...
   %--- Change header info on first and last year to match the data
   F(2,67:80)=hdB5;
...
disp('Hang on -- writing the output file...');
