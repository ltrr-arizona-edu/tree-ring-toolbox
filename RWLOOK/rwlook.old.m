...
while kwhsource==1;
    ...
    if ~any(ksource==[1 2]);
        uiwait(msgbox({'Only first two menu choices supported so far','Message'},'modal'));
    else;
        kwhsource=0;
    end;
end;
...
if strcmp(kmeas,'Yes');
%     
%     kmen1=menu('Choose data sources',...
%         'Series #1 and #2 from an rwmeas-formatted measurement storage file',...
%         'Series #1 from an rwmeas-formated storage file,  #2 from a .crn ITRDB-format chronology file');
    ...
    kmen1=menu('Choose data sources',...
        'Series #1 and #2 both from .rw files',...
        'Series #1 from .rw file, series #2 from 2-column .dat file',...
        'Series #1 and #2 both from 2-column .dat files');
    if kmen1==1;
        type1='rw'; type2='rw';
    elseif kmen1==2;
        type1='rw'; type2='dat';
...
if isempty(XX.id{1}) | isempty(YY.id{1});
...
while kk2==1;
    ...
    if  ~strcmp(kmeas,'Yes'); % neither series from rwmeas formatted .mat file
        ...
        else; % first series is from a .dat file
    ...
    if strcmp(type1,'dat') & strcmp(type2,'dat');
        Lneg = X1(:,2)<0;
        if any(Lneg);
            X1(Lneg,2)=0;
        end;
