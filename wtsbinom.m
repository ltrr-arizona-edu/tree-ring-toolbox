...
%
% Coefficients, or weights, of binomial filter of length a or period of 50% response 
% a years
%
%*** INPUT
%
...
%  kopt(2)==2   a is the number of weights (odd, central + both sides desired)
%
%*** OUTPUT
%
...
%    (e.g., n==3  goes with {.25 .50 .25}).  In reference, weights numbered 
%    0,1,2,...N.  Thus N==4 means 5 weights (n==5). Because of truncation (see below),
%    number of weights in filter b may be fewer than specified a%
%
...
elseif kopt==1; % input arg was desired wavelength (yr) of 50% response
   ...
   p50=a;
   %sigma = p50/6;  % standard deviat
   %nn =  2 * sigma .^2;
   nn = (p50/3).^2;
   % nn is half-width; must round up to nearest even integer
   nnpos = ceil(nn);
   if rem(nnpos,2)~=0;
      nnpos = floor(nn);
   end;
