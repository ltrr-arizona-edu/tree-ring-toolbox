...
case 'Edit';  % EDIT MODE
    ...
    elseif kedit1==4;  % import data from .rwl files produced outside the system
        ...
        if kmen1==1;
            editmode='Total';
            XT=rwlimport(XT,editmode);
            vbl='XT';
        else;
            editmode='Partial';
            XEL=rwlimport(XEL,editmode);
            vbl='XEL';
        end;
...
case 'Merge'; % MERGE TWO .MAT MEASUREMENT FILES
    ...
    nser2a = length(XT2.id); % number of total-width series
    nser2b = length(XEL2.id); % number of partial-width series
