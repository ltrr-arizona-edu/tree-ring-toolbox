...
kmen=menu('What do you want to do?','Measure','Edit','Merge 2 .mat files','Update fields');
if kmen==1;
    runmode='Measure';
elseif kmen==2;
    runmode='Edit';
...
case 'Edit';  % EDIT MODE
    ...
    elseif kedit1==5; % make or revise rwlset
        rwlset=rwlspecs(rwlset,XT.id,XEL.id);
        
        
    elseif kedit1==6;  % create .rw, etc files 
        pathdef = path1;
    ...
    switch kedit1;
    case{1,2,3,4,5}; % updatind needed after editing
         
        % Update span
        
        if kedit1 ~=5; % if not just modifying an rwlset
...
case 'Update'; % Update fields
    ...
    %-- RWLSET EXISTS, BUT EITHER TRIMEACH OR TRIMALL FIELDS ARE NOT THERE
    
    if exist('rwlset','var')==1;
        nrwlset=length(rwlset.name);
        if ~isfield(rwlset,'trimall');
            for kk = 1:nrwlset
                rwlset.trimall{kk}=[];
            end;
        end;
        if ~isfield(rwlset,'trimeach');
            for kk = 1:nrwlset
