...
%
% 	co052_rwl -- NO DATA TITLE LINE PROVIDED --               1ST ITRDB LINE MISSING  STD
% 	co052_rwl -- NO DATA TITLE LINE PROVIDED --               2ND ITRDB LINE MISSING  STD
% 	co052_rwl -- NO DATA TITLE LINE PROVIDED --               3RD ITRDB LINE MISSING  STD
%
...
if ~(isletter(char_last) | isspace(char_last) | isnumeric(char_last));
    file(end)=[];
end;

%--- Rev 4-12-04:  Remove last line if not a true data line, but some statistics line with no year
line_year=file{end}(7:10);
if all(isspace(line_year));
    file(end)=[];
end;
...
if ~isempty(findstr(line1,headdummy));
    headtype=2;
