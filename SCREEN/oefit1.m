function [s,r2,ob,of,f1,f2,way,k1]=oesplit1(Z,yr,nn,nranked,kopt)
% [s,r2,ob,of,f1,f2,way,k1]=oesplit1(Z,yr,nn,nranked,kopt);
%
...
%
% If no A or B model has all parameters significant, the order of pref
%  1-simplest model (fewest total number of parameters)
%
% 
...
%
%**************************************************************
%    DIAGNOSTICS MODE FIGURE WINDOWS
%
% zscores of input and output series
% A-model #1 , tsp of model and observed output 
% 
%
close all
a=NaN;
...
for n = 1:nmods;
	...
	txt3=['A-Model #',int2str(n)];
	txt4=['OE(',int2str(th(1,5)),',',int2str(th(1,8)),')'];
	title([txt3 txt4]);
	FA(n)=th(2,1);
	SA(n,2)=FA(n);
...
% Store information on top-ranked models -- minimum FPE models
...
if L; % if no model has all parameters significant
	...
	if any(LA) & any(LB); % At least one "B" model and one "A"
			% has all parameters significant
		jA = min(find(LA));
		jB=min(find(LB));
...
vare = th(1,1); % estimate variance of residuals
...
if kopt==2; % diagnostics mode
	[yh,fit] = compare(z,th,1);
	figure	
	plot(yr,z(:,1)+mn(1),yr,yh+mn(1),'--')
	...
	bord=int2str(th(1,5));
	ford=int2str(th(1,8));
	legend('Observed Output','Model Output')
	title(['OE(',bord,',',ford,')']);
	txt1=['Variance Explained Fraction = ',num2str(s)];
	text(yr(1)+5,yup1,txt1)
	present(th)
end
...
if kopt==2
	...
	set1=[' nn Z yr yrA yrB SA SB FA FB s ob of f1 f2 way k1 '];
