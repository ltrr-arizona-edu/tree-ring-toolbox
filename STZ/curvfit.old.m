...
% cfit1
%
% Interactive fitting of detrending curves.  Various curve fits possible,
% including spline and negative exponential.  Double detrending available
% if negative exponential is first detrending.
...
% year information
flmat=uigetfile('*.mat','.mat ringwidth storage file made with rwlinp.m: ');
flold=flmat;
...
while ksw1==1;  % while working on this core
 ...
 hf0=figure('Color','k','Units','normal','Position',[kfx,kfy,0.75,0.75]);
 plot(yrv,xv,'y');title(['Ring Width ',nms(scid,:)]);
...
%************** WHILE FIRST DETRENDING **************
 ...
 while ksw2==1;  
  ...
  hf1=figure('Color','k','Units','normal','Position',[kfx,kfy,0.75,0.75]);
  plot(yrvn,xvn,'y');title(['RingWidth ',nms(scid,:)]);
  ...
  figure(hf1);hold off;
  plot(yrvn,xvn,'y',yrvn,cvx(xind1:xind2),'m');
...
%********** WHILE BLOCKING OUT INTERVALS ***********
  ...
  while ksw3==1 & nblk~=6,
   ...
   if nblk==6, break; end
   figure(hf1);hold off;
   plot(yrvn,xvn,'y'); title(['Ring Width ',nms(scid,:)]);
   xlabel('Year');ylabel('Ring Width, (0.01 mm)');
   pause(1);
   ...
   if ksw3~=1,
    ...
    figure(hf1);hold off;
    plot(yrvn,xvn,'y',yrv(xind1:xind2),cvx(xind1:xind2),'m');
    title(['Ring Width ',nms(scid,:),tstr1]);
    xlabel('Year');ylabel('Ring Width, (0.01 mm)');
   end
   sgc=sgc+1;
  ...
  if ksw2~=1,
   ...
   if nq,
    rwin1=xv1(xind1:xind2)./cvx(xind1:xind2);
	 mnx=meannan(rwin1);  % mean for plotting 
    hf2=figure('Color','k','Units','normal','Position',[kfx,kfy,0.75,0.75]);
    plot(yrv(xind1:xind2),rwin1,'y',[yrv(xind1) yrv(xind2)],[mnx mnx],'w');
 ...
 if nfit1~=1; %  if not neg exp first fit, convert to zero mean additively
  ...
  S(scid,7)=mean(xgood);
   ...
   while nq~=1,
    ...
    if nfit2==9,
      hf3=figure('Color','k','Units','normal',...
	  'Position',[kfx,kfy,0.75,0.75]);
      plot(yrvn,xvn,'m',yrv(xind1:xind2),cvx2(xind1:xind2),'y');
    ...
    if nq,
     ...
     if nfit2==9,
       hf4=figure('Color','k','Units','normal','Position',[kfx,kfy,0.75,0.75]);
       rwin2=rwin1./cvx2(xind1:xind2);
       plot(yrv(xind1:xind2),rwin2,'y',[yrv(xind1) yrv(xind2)],...
			[meannan(rwin2) meannan(rwin2)],'w'),
       title([nms(scid,:),'- Index after double detrending']);
       xlabel('Year');ylabel('Index');
     end
    end
