function [struc,field,varlen,description,narrativefile] = vmap0read(filepath,filename,recordIDs,field,varlen)

%VMAP0READ reads a Digital Chart of the World file
%
% VMAP0READ reads a VMAP0 file.  The user may select the file interactively.
%
...
% fields from the file.  If recordIDs is a scalar or a vector of
% integers, the function returns the selected records.  If recordIDs is
% a cellarray of integers, all records of the associated fields are
% returned.
%
...
%  Written by:  W. Stumpf
%   $Revision: 1.4 $    $Date: 1997/03/28 23:11:45 $

if nargin >= 2

    ind = findstr(filename,'.');
%    if isempty(ind) & isunix ; filename = [filename '.']; end
    if isunix; filename = lower(filename); end 
...
%
% Check to see if these errors are still present when the next
% version of the DCW is released.
%
if strcmp('AEPOINT.PFT',filename)==1
