function [YP,Z,RSQ1,RSQ2,EV,nv,hf]=osr(X,Y,X1,mineigs,tcrit,kopt)
% Use: [YP,Z,RSQ1,RSQ2,EV,nv]=osr(X,Y,X1,tcrit,kopt)
% osr.m  -- orthogonal spatial regression
%
% D Meko 8-21-95
%
%
%*************** IN ARGS **********************************
%
% X (mm x p)r  tsm of predictors. Examples are (1) tree-ring
%		indices at p sites, and (2) single-tree reconstructions
%		of a climate variable at p trees; mm is "m-prime" in
%		Cook et al.
% Y (mm x q)r stm of predictands -- climate variables at q 
%		gridpoints or p regions
...
%******************  OUT ARGS ******************************
%
% YP (mm x q)r reconstructed predictand for calib period
% Z (mX1 x q)r reconstructed predictand for data X1
...
%
% STEPS
% - Convert X and Y to Zscores
% - PCA X and Y and delete unimportant compontents to reduce data sets
% - Regress predictand amplitudes on predictor amplitudes, further
...
%************  Convert X,Y to z-scores and store means and std devs
...
%
%
[RE,E,LE,SE,U,CE]=eigen1(Xz,1);
[RF,F,LF,SF,V,CF]=eigen1(Yz,1);
...
if kopt(2)==1; % use all predictands
	qq=q;
elseif kopt(2)==2; % eigenvalue of 1
	qq = sum(diag(LF)>1);
end
...
elseif kopt(3)==2; % sig test on coefficients -- use 95%
	...
	for kk = 1:qq;
		[atilde,t] = pcareg1(LEsub,Uz,Vz(:,kk),E(:,1:pp),tcrit);
		A(:,kk) = atilde;
