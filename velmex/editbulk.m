...
%
% Apply same change to multiple series: change the last year, insert a locally absent ring, 
% remove a locally absent ring, or delete the entire series.
%
%
%*** INPUT
%
...
elseif kchng==6; % Delete series
    strmess1={'You will be able to delete individual series from the .mat storage file.',...
            'You can select the series to be deleted.',...
            'Deleting them means the measurements for those series are gone for good!',...
            'If you delete series that are in an rwl set, you will need to revise that rwl set before using it to write rwl files.' };
...
if kchng ~=6;
    strmen1={'APPLY CHANGE TO ALL SERIES',...
            'CLICK ON SERIES TO BE CHANGED',...
            'CLICK ON SERIES TO BE EXCLUDED FROM CHANGE'};
    strmen2=['Click so that series to be changed marked with -Y'];
else;
    strmen1={'DELETE ALL SERIES -- CANNOT DO THIS',...
            'CLICK ON SERIES TO BE DELETED',...
            'CLICK ON SERIES TO BE EXCLUDED FROM DELETION'};
    strmen2=['Click so that series to be deleted marked with -Y'];
end;

kmen1=menu('Choose',strmen1);
...
if kmen1==1; % SAME CHANGE TO ALL SERIES
    ...
    if kchng==6; % if deletion mode
        error('CANNOT USE THIS FUNCTION TO DELETE ALL SERIES');
    end;
...
if isempty(jedit);
    if kchng~=6;
        ...
        uiwait(msgbox('No series have been marked for deletion','Message','modal')); 
    end;
    return;
end;
...
elseif kchng==6; % delete series
    % npick == number of series to be deleted
    % jedit == index to those series
...
if kchng~=6;  % if not in series-deletion mode
    for n = 1:npick;
        jthis=jedit(n);
