...
%-truncate series at either end
%-overwrite stretch of measurements with NaNs
%   For example, might want to do this if in checking measurements see that injury response for 8 years in a
%   part of series
%
%
%*** INPUT
...
if size(d{1},2)==2;
    editmode='Total';
    lab1='RING WIDTH (mm)';
    jcol=1; % data col, measured total width 
    jj=1;
elseif size(d{1},2)==4;
    editmode='Partial';
    lab1='EWW+LWW (mm)';
    jcol =3; % data col, computed total width, sum of EW and LW
    jj=[1 2 3];
end;
...
while kwh1==1;
    kmen1 = menu('What do you want to do?',...
        ...
        'CHANGE A MEASURED VALUE',...
        'TRUNCATE SERIES (LOP OFF LEADING OR ENDING YEARS)',...
        'ABORT THE CHANGE AND START OVER',...
        'OVERWRITE STRING OF MEASUREMENTS WITH NANS',...
    ...
    elseif kmen1==3; % CHANGE REAL RING TO FALSE
        if kdone==1;
            ...
            if kmen2==1; % keep same start year
                yrx = yrgo+((1:nyr)')-1;
                % remeas{kser}=[yrkey];  % NO NEED TO REMEASURE WHEN COMBINING RINGS
            elseif kmen2==2; % keep same end year
                yrx = yrgo+((1:nyr)');
                % remeas{kser}=[yrkey+1];% NO NEED TO REMEASURE WHEN COMBINING RINGS
            end;
            subfcn02(x0,yrx0,x,yrx,id{kser},lab1);
            kdone=1;
        end;
    ...
    elseif kmen1==5;% INSERT A STRETCH OF NANS 
        if kdone==1;
            strmsg1='CANNOT STACK CHANGE ON CHANGE: ABORT PREVIOUS CHANGE FIRST';
            uiwait(msgbox(strmsg1,'Message','modal'));
        else;
    ...
    elseif kmen1==7; % TRUNCATE SERIES AT EITHER END
        yrsp = yrx(end); 
        yrgo = yrx(1);
        syrs={num2str([yrgo yrsp])};
        txttit1=['Truncate series'];
        prompt={'Enter desired start and end years of truncated series:'};
        kwh2=1;
        while kwh2;
            def=syrs;
            dlgTitle=txttit1;
            lineNo=1;
            answer=inputdlg(prompt,dlgTitle,lineNo,def);
            yrs= str2num(answer{1});
            if yrs(1)<yrgo | yrs(2)>yrsp;
               uiwait(msgbox(['Years must be within range ' syrs{1}],'Enter Again','modal'));
           else;
               kwh2=0;
           end;
       end;
        ...
        % Truncate data
        Ltrunc=yrx>=yrs(1) & yrx<=yrs(2);
        yrx=yrx(Ltrunc);
        x=x(Ltrunc,:);
        X=X(Ltrunc,:);
        clear kwh2 Ltrunc
        subfcn02(x0,yrx0,x,yrx,id{kser},lab1);
    elseif kmen1==8; % ABORT THE CHANGE AND START OVER
        yrx=yrx0;
        x=x0;
        X=X0;
        remeas{kser}=[];
        ...
        figure(1);
        subfcn01(x0,yrx0,id{kser},lab1);
    elseif kmen1==9; % overwrite measurements with NaN
        if kdone==1;
            strmsg1='CANNOT STACK CHANGE ON CHANGE: ABORT PREVIOUS CHANGE FIRST';
            uiwait(msgbox(strmsg1,'Message','modal'));
        else;
            ...
            yrgo = yrx(1);
            syrkey='9999';
            txttit1=['Overwrite measurements with NaNs beginning with year: )'];

            % PROMPT FOR YEAR
            kwh=1;
            while kwh==1;
                prompt={'Enter First NaN Year','Enter last NaN year'};
                def={syrkey,syrkey};
                dlgTitle=txttit1;
                lineNo=1;
                answer=inputdlg(prompt,dlgTitle,lineNo,def);
                yrkey = str2num(answer{1});
                yrkeyoff = str2num(answer{2});
                nmiss=yrkeyoff-yrkey+1;
                syr =num2str(yrkey);
                if nmiss<=0;
                    uiwait(msgbox('Last year must be later than first, try again','Invalid Entry','modal'));
                    kwh=1;
                else;
                end
                if yrkey<yrgo | yrkeyoff>yrsp;
                    uiwait(msgbox('Specified years for overwrite outside range for series, try again','Invalid Entry','modal'));
                    kwh=1;
                else;
                    kwh=0;
                end;
            ...
            % Pointer to the segment to overwrite; and overwrite
            L=yrx>=yrkey & yrx<=yrkeyoff;
            x(L)=xkey;
            X(L,:)=Xkey;
        ...
        elseif  kmen1==10; % ACCEPT THE CHANGE AND QUIT
...
set(hp2(1),'Linestyle','-','Marker','o');
set(hp2(2),'Linestyle','-','Marker','^');
