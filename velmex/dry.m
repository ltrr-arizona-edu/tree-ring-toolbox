% dry.m -- version of wry being edited for EWW/LWW
...
yrorigin = 1601; % first year of existing measurements
yrstart=1601; % year of first partial ring to be measured
yroffset=yrstart-yrorigin;
% total, early, late width
x=repmat(NaN,800,1);
x1=repmat(NaN,800,1);
x2=repmat(NaN,800,1);
...
while kwh1==1;
    ...
    if count==16;  % One measured value is in buffer
        ...
        n=n+1; % Increment relative index for readings from QC; if first reading, n==1, etch
        % If earlywood
        if kwood==1 | n==0;;
        ...
        if n>1 &  w<0; % Check for negative difference -- meaning you reversed crank on velmex
            ...
            if n>1;
                ...
                yrthis = yrstart+n1-1;
                xtot=x1(n1)+x2(n1);
                str1=num2str([n1 yrthis x1(n1) x2(n1) xtot]); 
                if kwood==1;
                    disp(str1);
                end;
...
x1=trailnan(x1); % stripp off trailing NaNs
x2=trailnan(x2); % stripp off trailing NaNs
x=x1+x2;

nx = length(x);
yr = (1:nx)' - 1 +yrstart+yroffset;
