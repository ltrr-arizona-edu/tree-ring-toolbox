...
% 1. Store these three matrices in a  ".mat" :
%    1. A - the data array, with year as the first column
%    2. opts - the options array (see opts in variable list)
...
% 4. >pdgm1         to start program



% #################   FEATURES  ##########################################
%
% * Screen plots  of spectrum, raw periodogram, and confidence
...
%   386-Matlab)
%
% * Selective analysis of specified column (variable)
%   of a matrix of time series.
%   

% ##########  OTHER USER-WRITTEN MATLAB FUNCTIONS NEEDED #####
%
% LK025.m   -- for looking up chi-sq value at 2.5% level
% LK975.m   -- ... at 97.5% level
...
var=zeros(3,1); % to hold variances of original and tapered data, and
   ...
   % Screen prompt for first year and for series to analyze
   
ago = A(1,1); % first year in the input tsm
   
prompt={'Enter the col (after year column) to analyze'};
def={1};
tit='Choose series to analyze';
lineNo=1;
answer=inputdlg(prompt,tit,lineNo,def);
nnn=str2num(answer{1});
...
% Screen prompt for padded length
prompt={'Enter desired padded length'};
def={2^nextpow2(last-first+1)};
...
yover= 3*max(ysmhi)/4;    % shift 3/4 of way to top
...
plot(ff,ysm1(:),'-g',ff,ysmlo,'-.b',ff,ysmhi,'-.b',...
  ff,ysm2,'--b',ftemp+xover,wsave.*yfact+yover,'-b',...
  fline+xover,straight+yover,'-b')
...
text(.65,.8,['Region # ',int2str(nnn)],'sc');
