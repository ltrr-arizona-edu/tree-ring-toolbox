...
% rwmeas;
% Last revised 2010-10-10
% Updated by S. Brewster Malevich for Ubuntu Linux 10.04.01
%
...
%
% Rev042204:  naming convention loosened so that user need not stick with site-tree-core-order naming.  Before,
% needed name such as pdf04a3, which specifies site pdf, tree 4, core a, and the third measured segment from the inside.  
% Now, can use a nonsense name, as long as it is 8 or fewer characters.  For example, 68#rt-63  will work. The initial prompt for the
% program settings asks if you want the strict naming or the max-of-8-character naming.
%
% Rev060809: to optionally allow rwl and rw output data to be in thousandths of mm.  Through new optional argument,
% kdigits, in call to rwc2rwl or rwc2rw
...
kmen=menu('What do you want to do?','Measure','Edit','Merge 2 .mat files','Update fields','Abort');
if kmen==1;
    runmode='Measure';
elseif kmen==2;
    runmode='Edit';
...
case 'Edit';  % EDIT MODE
    ...
    if kedit1==3 | kedit1==4; % if importing data
        if kedit1==3;
        ...
        else; % kedit1==4, importing from .rwl file.  Assume new file
            kfile='New';
        end;
    end;
    filemode=kfile;
    ...
    if ~strcmp(kfile,'New');
        ...
        [XT,XEL,sinfo,rwlset,vlist,pf1]=subfcn08('mm');
    end;
    ...
    elseif kedit1==3;  % import data from a .rw, .eww or .lww file measured outside the system
        ...
        %--- TYPE OF DATA TO EDIT
        kmen1=menu('IMPORT WHAT?',...
            'TOTAL WIDTH',...
            'PARTIAL WIDTH');
        kdigits=menu('What units are the input data','Hundredths of mm','Thousandths of mm');
        if kmen1==1;
            editmode='Total';
            XT=rwimport(XT,editmode,kdigits);
            vbl='XT';
        else;
            editmode='Partial';
            XEL=rwimport(XEL,editmode,kdigits);
            vbl='XEL';
        end;
        
    elseif kedit1==4;  % import data from .rwl files produced outside the system
        %--- TYPE OF DATA TO EDIT
        kmen1=menu('IMPORT WHAT?',...
            'TOTAL WIDTH',...
            'PARTIAL WIDTH');
        ...
        if kmen1==1;
            editmode='Total';
            XT=rwlimport(XT,editmode,kdigits);
            vbl='XT';
        else;
            editmode='Partial';
            XEL=rwlimport(XEL,editmode,kdigits);
            vbl='XEL';
        end;
        
    elseif kedit1==5; % rename a series
        %--- TYPE OF DATA TO RENAME
        kmen1=menu('RENAME WHAT?',...
            'TOTAL WIDTH SERIES',...
            'PARTIAL WIDTH SERIES');
        if kmen1==1;
            editmode='Total';
            % Here the call to unwritten function that returns revised XT
            [XT,XEL,rwlset,kboth]=rwrename(XT,XEL,rwlset,editmode);
            vbl='XT';
        else;
            editmode='Partial';
            % Here the call to unwritten function that returns revised EL
            [XT,XEL,rwlset,kboth]=rwrename(XT,XEL,rwlset,editmode);  
            vbl='XEL';
        end;
        
        
    elseif kedit1==6;  % delete a series
        %--- TYPE OF DATA TO DELETE
        kmen1=menu('DELETE WHAT?',...
            'TOTAL WIDTH SERIES',...
            'PARTIAL WIDTH SERIES');
        if kmen1==1;
            editmode='Total';
            % Here the call to unwritten function that returns revised XT
             [XT,rwlset]=rwdelete(XT,rwlset,editmode);
            vbl='XT';
        else;
            editmode='Partial';
            % Here the call to unwritten function that returns revised EL
             [XEL,rwlset]=rwdelete(XEL,rwlset,editmode);
            vbl='XEL';
        end;
        
    elseif kedit1==7; % survey series
        rwsurvey(XT,XEL);
               
    elseif kedit1==8; % make or revise rwlset
        rwlset=rwlspecs(rwlset,XT,XEL);
        
        
    elseif kedit1==9;  % create .rw, etc files 
        pathdef = path1;
        ...
        pathout = answer{1};
        kdigits=menu('Choose desired units of rwl or rw file data','Hundredths of mm','Thousandths of mm');
        ...
        while kwh1==1;
            ...
            if kmen1==1; % .rwl files
                ...
                if kmen==1;
                    ftype='rw';
                    rwc2rwl(XT,XEL,pathout,ftype,rwlset,kdigits);
            ...
            elseif kmen1==2; % .rw, .eww or .lww files
                ...
                if kmen==1; % .rw files
                    ...
                    rwc2rw(XT,XEL,pathout,ftype,kdigits);
    ...
    switch kedit1;
        case{1,2,3,4,8}; % updating needed after editing
            
            % Recall that kedit1 choices were:
            % 1 'Edit measurements for single series',...
            % 2 'Bulk edit multiple series',...
            % 3 'Import .rw, .eww or .lww files         ',...
            % 4 'Import series from .rwl files        ',...
            % 5 'Rename a series',...
            % 6 'Delete a series',...
            % 7 'survey series' -- NO UPDATING NEEDED
            % 8 'Create or revise an rwl specification set',...
            % 9 'Write output data files'-- NO UPDATING NEEDED); 
            
            if kedit1 ~=8; % if not just modifying an rwlset
                
                eval([vbl ' = subfcn07(' vbl ');']); % update span
                
                eval([vbl ' = subfcn02(' vbl ');']); % Sort the revised data so that series alphabetically ordered
                
                % Update X.summary (string array)
        ...
        case{5} % renaming   series
            % Must 1) update span, 2) sort the revised alphabetically, 3) update summary, 4) update rwlset, 5) update sinfo 
            % Must optionally do for XT and XEL if flag from functions says BOTH were modified
            % Must save results
            
            vblinit=vbl; % store valie of vbl for later re-initializing
           % Sort the revised data so that series alphabetically ordered
            eval([vbl ' = subfcn02(' vbl ');']);  % key series-type 
            if kboth==1;
                if strcmp(editmode,'Partial');
                    vbl='XT';
                    eval([vbl ' = subfcn02(' vbl ');']); % other series-type
                elseif strcmp(editmode,'Total');
                    vbl='XEL';
                    eval([vbl ' = subfcn02(' vbl ');']); % other series-type
                else; 
                    error ('Invalid editmode');
                end; % if strcmp(vbl,'XEL');
            end;
            vbl=vblinit; % re-initialize vbl to either 'XT' or 'XEL', whichever it was before this section
            ...
            if kboth==1;
                if strcmp(editmode,'Partial');
                    vbl='XT';
                    eval([vbl ' = subfcn07(' vbl ');']); % update span of other measurement type
                elseif strcmp(editmode,'Total');
                    vbl='XEL';
                    eval([vbl ' = subfcn07(' vbl ');']); % update span of other measurement type
                else; 
            ...
            % Update .summary (string array)
                if strcmp(editmode,'Total');
                    eval([vbl ' = subfcn03(' vbl ',1);']); % the "1" means total ring width
                    if kboth==1;
                        eval([vbl ' = subfcn03(' vbl ',2);']); % the "2" means partial ring width
                    end;
                elseif strcmp(editmode,'Partial');
                    eval([vbl ' = subfcn03(' vbl ',2);']);
                    if kboth==1;
            ...
            case{6} % deleting   series
                ...
                % Update the rwlset
                rwlset=subfcn09(XT,XEL,rwlset);
                
                % Save results -- call subfcn04
                subfcn04(filemode,pf1,XT,XEL,vlist,rwlset,sinfo);
                
                
            otherwise; % surveyed files, wrote output files, no updating needed
        end;
    ...
    case 'Measure';  % MEASUREMENT MODE
        ...
        prompt={'Enter your initials','Enter the units for measurments as stored in files',...
                ...
                'Enter series-naming convention: "stco" (site-tree-core-order) or "max8" (no restriction, by max of 8 chars)'};
        def={'dude','mm','2000','200','max8'};
        ...
        name_conv = answer{5};
        jtemp=strmatch (name_conv,{'stco','max8'},'exact');
        if isempty(jtemp);
            error('Must enter either stco or max8 as the naming convention');
            return;
        end;
        name_conv=lower(name_conv);
            
        
        kmen = menu('Choose','Start new site',...
            'Operate on existing site','Abort');
        switch kmen;
            case 1; 
                filemode='New';
    ...
    if strcmp(filemode,'New');
        ...
        jser=1; % first series
        if strcmp(name_conv,'stco'); % if restricting series name to follow site-tree-core-order convention
            nameser = subfcn01; % Call subfcn01.m to  name series
        else;
             nameser = subfcn01a; % Call subfcn01a.m to  name series
        end;
...
case 'Merge'; % MERGE TWO .MAT MEASUREMENT FILES
    %error('''Merge'' not yet implemented');
      
    
    %---- GET THE DATA FROM THE FIRST OF TWO .MAT STORAGE FILES
    
    pause(0.1);
    [file1,path1]=uigetfile('*.mat','First of two .mat files to be merged');
    pf1=[path1,file1];
    eval(['s=load(''' pf1 ''');']);
    if ~all([isfield(s,'vlist')  isfield(s,'XEL')  isfield(s,'XT')  isfield(s,'sinfo') isfield(s,'rwlset')]);
        error([pf1 ' does not contain XEL, XT , sinfo, rwlset and vlist']);
    ...
    clear s;
    % Store the data fron the first file
    eval(['load ' pf1 '  XT XEL sinfo rwlset vlist;']); 
    XT1=XT;
    XEL1=XEL;
    sinfo1=sinfo;
    rwlset1=rwlset;
    vlist1=vlist;
    clear XT XEL sinfo rwlset vlist;
    
    
    %---- GET THE DATA FROM THE    SECOND  OF TWO .MAT STORAGE FILES
    
    [file1,path1]=uigetfile('*.mat','Second of two .mat files to be merged');
    pf1=[path1,file1];
    eval(['s=load(''' pf1 ''');']);
    if ~all([isfield(s,'vlist')  isfield(s,'XEL')  isfield(s,'XT')  isfield(s,'sinfo') isfield(s,'rwlset')]);
        error([pf1 ' does not contain XEL, XT , sinfo, rwlset and vlist']);
    ...
    clear s;
    % Store the data from second file
    eval(['load ' pf1 '  XT XEL sinfo rwlset vlist;']); 
    XT2=XT;
    XEL2=XEL;
    sinfo2=sinfo;
    rwlset2=rwlset;
    vlist2=vlist;
    clear XT XEL sinfo rwlset;
    
    
    %---   MERGE THE XT STRUCTURES
    
    % First file tally
    nrwlset1=length(rwlset1.name); % number of rwl sets
    if ~isempty(XT1.id{1});
        nser1a = length(XT1.id); % number of total-width series
    else;
        nser1a=0;
    end;
    if ~isempty(XEL1.id{1});
        nser1b = length(XEL1.id); % number of total-width series
    else;
        nser1b=0;
    end;
    ...
    % Second file tally
    nrwlset2=length(rwlset2.name); % number of rwl sets
    if ~isempty(XT2.id{1});
        nser2a = length(XT2.id); % number of total-width series
    else;
        nser2a=0;
    end;
    if ~isempty(XEL2.id{1});
        nser2b = length(XEL2.id); % number of total-width series
    else;
        nser2b=0;
    end;
    ...
    % Merged file
    nsera=nser1a+nser2a; % number of total-width series in merged
    nserb=nser1b+nser2b; % number of partial-width series in merged
    nrwlset=nrwlset1+nrwlset2; % expected number of  rwl sets in merged series
    ...
    elseif nrwlset1==0;
        ...
        rwlset.name=[rwlset1.name rwlset2.name];
        rwlset.describe=[rwlset1.describe rwlset2.describe];
        rwlset.when=[rwlset1.when rwlset2.when];
        rwlset.trimall=[rwlset1.trimall rwlset2.trimall];
        rwlset.idnames=[rwlset1.idnames rwlset2.idnames];
        rwlset.trimeach=[rwlset1.trimeach rwlset2.trimeach];
    end;
        
           
    if isempty(XT1.id{1}) & ~isempty(XT2.id{1});
        XT.id=XT2.id;
        XT.data=XT2.data;
        XT.span=XT2.span;
        XT.who=XT2.who;
        XT.when=XT2.when;
        XT.remeasure=XT2.remeasure;
    elseif isempty(XT2.id{1})  & ~isempty(XT1.id{1});
        XT.id=XT1.id;
        XT.data=XT1.data;
        XT.span=XT1.span;
        XT.who=XT1.who;
        XT.when=XT1.when;
        XT.remeasure=XT1.remeasure;
    elseif isempty(XT1.id{1}) & isempty(XT2.id{1});
        XT.id=XT1.id;
        XT.data=XT1.data;
        XT.span=XT1.span;
        XT.who=XT1.who;
        ...
        XT.id=[XT1.id XT2.id];
        XT.data=[XT1.data XT2.data];
        XT.span=[XT1.span XT2.span];
        XT.who=[XT1.who XT2.who];
        XT.when=[XT1.when XT2.when];
        XT.remeasure=[XT1.remeasure XT2.remeasure];
    end;
    ...
    elseif isempty(XT1.id{1}) & isempty(XT2.id{1});
        ...
        XT.who=[XT1.who XT2.who];
        XT.when=[XT1.when XT2.when];
        XT.when=[XT1.span XT2.span];
        XT.remeasure=[XT1.remeasure XT2.remeasure];
    end; 
    
    if isempty(XEL1.id{1}) & ~isempty(XEL2.id{1});
        XEL.id=XEL2.id;
        XEL.data=XEL2.data;
        XEL.span=XEL2.span;
        XEL.who=XEL2.who;
        XEL.when=XEL2.when;
        XEL.remeasure=XEL2.remeasure;
    elseif isempty(XEL2.id{1})  & ~isempty(XEL1.id{1});
        XEL.id=XEL1.id;
        XEL.data=XEL1.data;
        XEL.span=XEL1.span;
        XEL.who=XEL1.who;
        XEL.when=XEL1.when;
        XEL.remeasure=XEL1.remeasure;
    elseif isempty(XEL1.id{1}) & isempty(XEL2.id{1});
        XEL.id=XEL1.id;
        XEL.data=XEL1.data;
        XEL.span=XEL1.span;
        XEL.who=XEL1.who;
        ...
        XEL.id=[XEL1.id XEL2.id];
        XEL.data=[XEL1.data XEL2.data];
        XEL.who=[XEL1.who XEL2.who];
        XEL.when=[XEL1.when XEL2.when];
        XEL.span=[XEL1.span XEL2.span];
        XEL.remeasure=[XEL1.remeasure XEL2.remeasure];
    end;
    ...
    % Units check
    Lunits=(strcmp(XEL1.units,XEL2.units)) & (strcmp(XT1.units,XT2.units)) & (strcmp(XEL1.units,XT1.units));
    if ~all(Lunits);
        error(['The .units string not identical in all data being merged']);
    end
    clear Lunits;
    XT.units = XT1.units;
    XEL.units = XEL1.units;
    ...
    % Save results -- call subfcn04
    [file1,path1]=uiputfile('*.mat','Desired filename for merged storage file');
    pf1=[path1,file1];
    subfcn04('New',pf1,XT,XEL,vlist,rwlset,sinfo);
    
case 'Update'; % Update fields
    
    pause(0.1);
    [file1,path1]=uigetfile('*.mat','.mat file whose fields are to be updated');
    pf1=[path1,file1];
    eval(['s=load(''' pf1 ''');']);
    if ~all([isfield(s,'vlist')  isfield(s,'XEL')  isfield(s,'XT')  isfield(s,'sinfo') ]);
        error([pf1 ' does not contain XEL, XT , sinfo, and vlist']);
    end
    XT=s.XT;
    XEL=s.XEL;
    sinfo=s.sinfo;
    ...
    if isfield(s,'rwlset'); % rwlset exists in the file
        rwlset=s.rwlset;
        if ~all([isfield(rwlset,'name')  isfield(rwlset,'describe')  isfield(rwlset,'when')  isfield(rwlset,'idnames') ]);
            error(['rwlset does not contain fields name, describe, when and idnames']);
        end
        nrwlset=length(rwlset.name);
        if ~isfield(rwlset,'trimall'); % rwlset missing the field trimall
            if isempty(rwlset.idnames{1});
                rwlset.trimall={[]};
            else;
                for kk = 1:nrwlset
                    rwlset.trimall{kk}=[];
        ...
        if ~isfield(rwlset,'trimeach'); % rwlset missing the field trimeach
            if isempty(rwlset.idnames{1});
                rwlset.trimall={[]};
            else;
                for kk = 1:nrwlset
                    nseries = length(rwlset.idnames{kk});
                    rwlset.trimeach{kk}=cell(1,nseries);
                end;
            end;
        end;
    else; % rwlset does not exist in the file
        rwlset.name{1}=[];
        rwlset.describe{1}=[];
        rwlset.when{1}=[];
        rwlset.trimall{1}=[];
...
% ******  SUBFUCTION 1  -- getting and checking the series name -- restricteed naming
...
% ******  SUBFUCTION 1A -- getting and checking the series name-- generic naming

function nameser = subfcn01a;

prompt={'Enter the series ID:'};
def={'XXXXXXXX'};
dlgTitle='Series ID (max of 8 chars, no spaces (e.g., 68#12-tr)';
lineNo=1;
answer=inputdlg(prompt,dlgTitle,lineNo,def);
d1=answer{1};
d2=deblank(d1);
...
if isempty(X.id{1}); % if no series
    X.summary=[];
else
...
pause(0.1)  % Needed to get by a bug with uiputfile.
...
%---- SUBFUNCTION 09   UPDATE RWLSET TO ENSURE THAT IDS REFERRED TO IN SETS ARE CURRENTLY
% EXISTING  XT OR XEL SERIES

function rwlset=subfcn09(XT,XEL,rwlset);

% How many rwlsets are there?
if isempty(rwlset.idnames{1});
    nsets=0;
else;
...
if nsets ~=0;  % if at least one existing rwlset
    id1 = XT.id;  % ids of total-width series
    id2= XEL.id;  % % ids of partial-width series
    for m = 1:nsets; % loop over rwlsets
        thesenames=rwlset.idnames{m};
        n1 = length(thesenames); % number of names in this set
        if isempty(id1{1}); % if no XT series 
            L1 = logical(zeros(1,n1));
        else;
            L1=ismember(thesenames,id1);
        end;
        if isempty(id2{1}); % if no XEL series 
            L2 = logical(zeros(1,n1));
        else;
            L2=ismember(thesenames,id2);
        end;
        L3=L1 | L2; % 1 if id in either of XT or XEL, 0 otherwise
        rwlset.idnames{m}=rwlset.idnames{m}(L3); % update id names
        rwlset.trimeach{m}=rwlset.trimeach{m}(L3); % update individual time windows
    end;
    
    % Delete any set that now has no series 
    Lerase= logical(zeros(nsets,1));
    for m =1:nsets;
        if isempty(rwlset.idnames{m});
            Lerase(m)=1;
        end;
    end;
    if any(Lerase);
        rwlset.name(Lerase)=[];
        rwlset.describe(Lerase)=[];
        rwlset.when(Lerase)=[];
        rwlset.trimall(Lerase)=[];
        rwlset.trimeach(Lerase)=[];
        rwlset.idnames(Lerase)=[];
    end;
    
    % Aphabetically arrange the series in each rwlset
    % How many rwlsets are there?
    if isempty(rwlset.idnames{1});
        nsets=0;
    else;
    ...
    for m =1:nsets;
        thesenames = rwlset.idnames{m};
        thesetrim = rwlset.trimeach{m};
        
       if length(thesenames)>1;
            [thesenames,j]=sort(thesenames);
            thesetrim=thesetrim(j);;
            rwlset.idnames{m}=thesenames;
            rwlset.trimeach{m}=thesetrim;
        else;
            % No action needed to sort one
        end;
    end;
else; % no existing rwlsets, no need for any revision to rwlset
end; %  if nsets ~=0;
