function [yearend,nx,sx]=pltdep3a(pfs,jcols,kopt,YRS,thresh,plotinf)
% pltdep3a:  Seasonal bar chart of time series of departures, with runs summary
% CALL: [yearend,nx,sx]=pltdep3a(pfs,jcols,kopt,YRS,thresh,plotinf)
%
% Meko 9-10-97
%
%****************** IN ***********************************
%
% pfs{?)}c paths & filenames of .mat files holding climate series
%   Example: {'d:\outfls\coolbig.mat','d:\outfls\warmbig.mat'}... for
...
% thresh (1 x nseas)r (or empty) -- monthly thresholds
%    If kopt(3) is 1 or 2, thresh may be left []
%    If kopt(3) 3-7, thresh is a rv of length nseas, each corresponding to a seas. For example,
%       [30 30  sets threhold at 30th percentile  if
...
for n =1:nfiles;
   ...
   % Pull data for plot period and put in Z1
   L1= yr>=YRS(1,1) & yr<=YRS(1,2); % years to plot
   Z1=X(L1);
      
   % Pull data for stats period and put in Z2
   L2= yr>=YRS(2,1) & yr<=YRS(2,2); % years for stats
   Z2=X(L2);
   
   % Check that no NaN data in plot period for this series
   if any(isnan(Z1));
      error(['Season ' int2str(n) ':NaN data in period for analysis and plotting']);
   end
   
   % Get seasonal long term' mean, median and std dev for stats period
   xmean=nanmean(Z2);
   xmed=nanmedian(Z2);
   xstd = nanstd(Z2);
   ...
   X1 = Z1;
   yr2=yrplot;
   x2=X1; % again rename for convenience
   m1=length(x2); % number of years of plot data
   ...
   ybase=Y; % renamed baseline, for convenience
   x2=x2-ybase; % Subtract seasonal baseline
   ...
   [Px,nx,sx]=runs(x2,yrplot,xc,ksign);
   
   % Convert sequential observation of end of run to year
   yearend=Px(:,2);  % ending sequential obs of run
   ...
   [temp,itemp]=min(nx(ftemp)); % if tie, look for shortest with same runsum
   isum1=ftemp(itemp);
   sum1=sx(isum1);
   long1=nx(isum1);  % run-length of run with greatest run-sum
   str2a=sprintf('%g, ending in year %4.0f',sum1,yearend(isum1));
   ...
   Ltemp=nx==num1;
   ftemp=find(Ltemp);
   [temp,itemp]=max(sx(ftemp)); % if tie in run-length, pick one with highest sx
   inum1=ftemp(itemp);
   num1=nx(inum1);
   sum2=sx(inum1); % run-sum of run with longest length
   str2b=sprintf('%g years, ending in %4.0f',num1,yearend(inum1));
   ...
   % BEGIN MAKING PLOTS
   figure(n);  % new figure for each season
   h1=bar(x2hi);
   ...
   % Put gridlines at 5-yr intervals
   L5=rem(yrplot,5)==0;
   timet = (1:m1)';
   set(gca,'Xtick',timet(L5),'XtickLabel','','XGrid','on');
   
   % Annotate with year, every 10 years
   ybot=get(gca,'YLim');
   ybot=ybot(1); 
   for nn = 1:m1;
      if rem(yr2(nn),10)==0;
         hbot(nn)=text(nn,ybot,int2str(yr2(nn)),...
   ...
   case 'Yes';
      ...
      for nn = 1:m1;
         if rem(yr2(nn),10)==0;
            oldpos=get(hbot(nn),'Position');
            oldpos(2)=ybot;
            set(hbot(nn),'Position',oldpos);
         end
      end
   case 'No';
   case 'Cancel';
   ...
   titln1=[sernam ': ' seasons{n} ' '  dtype ' Departures, '  titlab1 ];
   ...
   ktemp=questdlg(['Want an ascii file summarizing ' seasons{n} ' runs?']);
   switch ktemp
   case 'No';
   case 'Cancel';
   case 'Yes'
      [file2,path2]=uiputfile('*.txt',['Ascii table of ' seasons{n} ' runs']);
      pf2=[path2 file2];
      fid2=fopen(pf2,'w');
      ...
      head5=' Rank  End Year    Sum   Length(yrs ) ';
      nlist=(1:length(nx))';
      ...
      Z=[nlist  yearend sx nx];
      Z=Z(1:ans1,:);
      
      fmt1='%3.0f \t%4.0f \t%g\t(%2.0f)\n';
